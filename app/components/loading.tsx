...
const Loading = ({ animationType }: {animationType?:string}) => {
  ...
  const animationDelays = characters.reduce<{[key: string]:string}>((acc, char, index) => {
    acc[char] = `${step * index}s`;
    return acc;
  }, {});

  return (
    <div className="flex items-center justify-center pb-0 text-[240px]">
      <style>
        {`
      ...
      {Object.entries(animationDelays).map(([key, delay]) => (
        <div
          key={key}
          className={`${key}`}
          style={{ ...letterStyle, animationDelay: delay }}
        >
          {key}
        </div>
      ))}
